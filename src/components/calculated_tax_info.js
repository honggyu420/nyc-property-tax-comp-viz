...
class CalculatedTaxInfo extends Component {
  calculateTaxBill(property_tax_info) {
    ...
    let use_trans =
      tax_class === "2" &&
      property_tax_info.curtrntot &&
      property_tax_info.curtrntot !== "0" &&
      property_tax_info.curtrntot < property_tax_info.curtxbtot;
    let assessed_total_val = use_trans ? property_tax_info.curtrntot : property_tax_info.curtxbtot;
    let assessed_exemption = use_trans ? property_tax_info.curtrnextot : property_tax_info.curtxbextot;
    ...
    if (tax_class.indexOf("1") > -1 || tax_class.indexOf("2") > -1) {
      ...
      let calculated_string = numWithCommas(assessed_total_val);

      if (assessed_exemption !== "0") {
        calculated_string = "(" + calculated_string + " - " + numWithCommas(assessed_exemption) + ")";
      ...
      calculated_string += " x " + (tax_rate * 100).toFixed(3) + "% = " + numWithCommas(estimated_tax_bill.toString());

      calculated_tax_results.push(
        <h4>
          estimated 2020 tax bill: <b>{calculated_string}</b>
        </h4>
      );
      ...
      calculated_tax_results.push(
        <h4>
          effective tax rate: <b>{tax_rate_against_market_val}%</b>.
        </h4>
      );
    }
